...
const AiTutor: React.FC<AiTutorProps> = ({ mode, currentTopic }) => {
  ...
  useEffect(() => {
    setMessages([{
        ...
        text: mode === AppMode.EXAM 
          ...
          : `Hello! I'm your Physics 4C AI Tutor. Topic: **${currentTopic}**.\n\nAsk me anything or say "Draw a diagram of..."`,
        timestamp: Date.now()
    }]);
  ...
  // --- Custom Math Parser ---
  // Splits text by $$...$$ (Block) and $...$ (Inline) to render Equation
  const renderMessageWithMath = (text: string) => {
    ...
    return blockParts.map((part, i) => {
      ...
      if (part.startsWith('$$') && part.endsWith('$$')) {
        return <Equation key={`block-${i}`} latex={part.slice(2, -2)} />;
      }

      // 2. Inside text segments, split by Inline Math: $ ... $
      const inlineParts = part.split(/(\$[^\$]+?\$)/g);
      
      return (
        <span key={`text-block-${i}`}>
          {inlineParts.map((subPart, j) => {
            if (subPart.startsWith('$') && subPart.endsWith('$')) {
               return <Equation key={`inline-${i}-${j}`} latex={subPart.slice(1, -1)} inline />;
            }
            // Render regular text with ReactMarkdown to preserve bold/italic/lists
            // Using span as 'p' component to avoid breaking inline flow
            return (
                <span key={`md-${i}-${j}`} className="inline-markdown">
              ...
              <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[90%] md:max-w-[80%] rounded-2xl px-5 py-4 ${
                  ...
                  <div className="prose prose-sm max-w-none dark:prose-invert leading-relaxed">
                     {renderMessageWithMath(msg.text)}
                  </div>
                </div>
              </div>
            );
