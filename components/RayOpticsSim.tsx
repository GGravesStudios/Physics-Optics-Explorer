...
const RayOpticsSim: React.FC = () => {
  ...
  const singleOpticResult = useMemo(() => {
    ...
    // Lens/Mirror Eq: 1/p + 1/q = 1/f -> q = pf / (p-f)
    let q = Infinity;
  ...
  const rays = useMemo(() => {
    ...
    return r;
  }, [systemMode, objDist, objHeight, focalLengthMag, lens2F, lensSep, slabIndex, slabThickness, opticType, subType, singleOpticResult]);
                ...
                <div className="space-y-4">
                    <div className="group">
                        <div className="flex justify-between text-sm mb-1 group-hover:text-amber-700 transition-colors">
                            <span className="text-slate-700 font-medium">Distance (<Equation latex="p" inline />)</span>
                    ...
                    <div className="group">
                        <div className="flex justify-between text-sm mb-1 group-hover:text-amber-700 transition-colors">
                            <span className="text-slate-700 font-medium">Height (<Equation latex="h" inline />)</span>
              ...
              <div>
                  ...
                  {systemMode === 'single' && subType !== 'plane' && (
                     <div className="mt-3 group">
                        <div className="flex justify-between text-sm mb-1 group-hover:text-amber-700 transition-colors">
                            <span className="text-slate-700 font-medium">Focal Length (<Equation latex="f" inline />)</span>
                      ...
                      <div className="space-y-4 mt-3">
                         <div className="group">
                            <div className="flex justify-between text-sm mb-1 group-hover:text-blue-700 transition-colors">
                                <span className="text-slate-700 font-medium">Lens 1 (<Equation latex="f_1" inline />)</span>
                         ...
                         <div className="group">
                            <div className="flex justify-between text-sm mb-1 group-hover:text-blue-700 transition-colors">
                                <span className="text-slate-700 font-medium">Lens 2 (<Equation latex="f_2" inline />)</span>
                         ...
                         <div className="group">
                            <div className="flex justify-between text-sm mb-1 group-hover:text-slate-700 transition-colors">
                                <span className="text-slate-700 font-medium">Separation (<Equation latex="d" inline />)</span>
                  ...
                  {systemMode === 'slab' && (
                      <div className="space-y-4 mt-3">
                         <div className="group">
                            <div className="flex justify-between text-sm mb-1 group-hover:text-amber-700 transition-colors">
                                <span className="text-slate-700 font-medium">Index (<Equation latex="n" inline />)</span>
                         ...
                         <div className="group">
                            <div className="flex justify-between text-sm mb-1 group-hover:text-amber-700 transition-colors">
                                <span className="text-slate-700 font-medium">Thickness (<Equation latex="t" inline />)</span>
           ...
           <div className="bg-slate-800 text-slate-200 p-4 rounded-xl text-xs shadow-inner font-mono border border-slate-700">
              ...
              {systemMode === 'single' && (
                <ul className="space-y-2">
                    <li className="flex justify-between">
                        <span className="text-slate-400">Position (<Equation latex="q" inline />):</span>
              ...
              {systemMode === 'two-lens' && (
                <ul className="space-y-1.5">
                    <li className="flex justify-between">
                        <span className="text-slate-400">Image 1 (<Equation latex="q_1" inline />):</span>
                        <span>{isFinite(twoLensResult.q1) ? twoLensResult.q1.toFixed(1) + ' cm' : 'Infinity'}</span>
                    </li>
                    <li className="flex justify-between">
                        <span className="text-slate-400">Obj for L2 (<Equation latex="p_2" inline />):</span>
                    ...
                    <li className="flex justify-between border-t border-slate-700 pt-1 mt-1">
                        <span className="text-slate-300">Final Image (<Equation latex="q_2" inline />):</span>
                        <span className="text-amber-300 font-bold">{isFinite(twoLensResult.q2) ? twoLensResult.q2.toFixed(1) + ' cm' : 'Infinity'}</span>
                    </li>
                    <li className="flex justify-between">
                        <span className="text-slate-400">Total Mag (<Equation latex="M" inline />):</span>
        ...
        <div className="lg:col-span-8 bg-slate-50 border border-slate-200 rounded-xl overflow-hidden relative min-h-[400px] shadow-sm">
           <svg viewBox="-400 -250 800 500" className="w-full h-full preserve-3d absolute inset-0" style={{ transform: 'scale(1, -1)' }} aria-label="Optics Simulation Canvas">
                ...
                {systemMode === 'single' && (
                    <g>
                        {subType === 'plane' ? (
                            <line x1="0" y1="-150" x2="0" y2="150" stroke="#64748b" strokeWidth="4" />
                        ) : (
                            opticType === 'lens' ? (
                                subType === 'converging' 
                                ? <path d="M 0 -150 Q 30 0 0 150 Q -30 0 0 -150" fill="#bae6fd" stroke="#0284c7" strokeWidth="2" opacity="0.6"/>
                                : <path d="M 0 -150 Q -20 0 0 150 L 0 -150 M -10 -150 Q 10 0 -10 150 L 10 150 L 10 -150" fill="#cffafe" stroke="#0891b2" strokeWidth="2" opacity="0.6"/>
                            ) : (
                                subType === 'converging' 
                                ? <path d="M -15 -150 Q 15 0 -15 150" fill="none" stroke="#64748b" strokeWidth="4" />
                                : <path d="M 15 -150 Q -15 0 15 150" fill="none" stroke="#64748b" strokeWidth="4" />
                            )
                        )}
